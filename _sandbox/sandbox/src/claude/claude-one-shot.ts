...
import type { OneShotAsk } from "../agent/adapter.js";
import { isFailureSentence } from "../agent/failure-sentences.js";
import { type HarnessCredentials, harnessEnv, resolveHarnessCredentials } from "../agent/harness-credentials.js";
...
const runOnHarness = async (params: {
...
export const claudeOneShot = async (services: Services, ask: OneShotAsk): Promise<string> => {
    ...
    return runOnHarness({ prompt: ask.prompt, cwd: ask.cwd, model: ask.model, credentials: resolved.credentials, signal: ask.signal });
};

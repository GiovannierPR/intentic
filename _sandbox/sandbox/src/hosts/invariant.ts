 ...
 * they are instances of (peers/invariant.ts). What is left in this directory is the door's own words, the Devices view's readings (served from memory and refreshed behind the answer, each carrying its own capturedAt), the command gate's judgement per call, and the setup-time seed, which arms once and burns. */
